 ...
 * The AT-2410 ratchet.
 *
 * A composer that owns its Enter with a `document` keydown listener gets NOTHING from the generic
 * `forceTriggerEnterActionForBreakLines` fallback — `resolveDictationSubmit` returns null and a
 * held dictation is inserted into a field that never sends. That failure is invisible: the mic
 * works, the transcript appears, and only the send is missing, so it reads as "the transcript
 * landed, I must still press the button" rather than as a bug.
 *
 * The comment popup shipped that way and it is the comment surface for ~17 entry points (the feed
 * interaction bar, the task/goal comment buttons, Suggested, Follow-up, the workflow modals),
 * i.e. everything except the Chat DV composer. This list is the three composers that post a
 * message somewhere; each must hand its own send action down. It is deliberately an explicit list
 * rather than a heuristic over every Enter-handling input — plenty of them (description fields,
 * instruction editors) take Enter as a newline and must NOT auto-send.
 */
describe('every composer that posts a message submits its dictations (AT-2410)', () => {
    const COMPOSERS = [
        ['the comment popup', '../../UIComponents/FloatModals/RichCommentModal/EditForm.js'],
        ['the chat composer', '../../ChatsView/ChatDV/EditorView/ChatInput.js'],
        ['the MyDay assistant line', '../../MyDayView/AssistantLine/AssistantOptions/AssistantOptions.js'],
    ]

    test.each(COMPOSERS)('%s hands CustomTextInput3 an onDictationSubmit', (_name, relativePath) => {
        const composer = fs.readFileSync(path.join(__dirname, relativePath), 'utf8')
