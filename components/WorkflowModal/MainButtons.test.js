...
describe('MainButtons layout', () => {
    ...
    it('shows only the backward action once a task is done, without the gap it reserved (AT-2501)', () => {
        const tree = renderer.create(
            <MainButtons
                compact
                hideForwardButton
                ...
                selectedCustomStep={false}
                onDonePress={jest.fn()}
                backwardStepName="Second review"
            />
        )

        expect(tree.root.findAllByType('ForwardButton')).toHaveLength(0)
        expect(tree.root.findByType('BackwardButton').props.targetStepName).toBe('Second review')
        expect(StyleSheet.flatten(tree.root.findByType('BackwardButton').props.buttonStyle)).toMatchObject({
            flex: 1,
            marginRight: 0,
        })
    })

    it('closes the stacked gap under a sole backward action on narrow screens (AT-2501)', () => {
        const tree = renderer.create(
            <MainButtons
                narrow
                compact
    ...
    it('keeps the forward action for every caller that does not ask for it to go', () => {
        // The long-press WorkflowModal also reports DONE_STEP on its legacy done branch and must
        // keep rendering both buttons, so the suppression is a prop and never `currentStep`.
        const tree = renderer.create(
            <MainButtons currentStep={-2} selectedCustomStep={false} onDonePress={jest.fn()} />
        )

        expect(tree.root.findAllByType('ForwardButton')).toHaveLength(1)
        expect(tree.root.findAllByType('BackwardButton')).toHaveLength(1)
    })
})
