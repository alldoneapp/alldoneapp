...
describe('GmailLabelingSettings helpers', () => {
    ...
    test('normalizes and sanitizes rule postLabelPrompt values', () => {
        ...
        expect(normalized.model).toBe('MODEL_GPT6_LUNA')
        expect(sanitized.model).toBe('MODEL_GPT6_LUNA')
    ...
    test('builds default label preview with project labels, Ads, and project follow-ups', () => {
        ...
        expect(preview.labelDefinitions[1].postLabelPrompt).toContain('followUpType is "actionable"')
        expect(preview.labelDefinitions[1].postLabelPrompt).toContain('followUpType is "informational"')
    ...
    test('builds default project follow-up prompt with the label name', () => {
        ...
        expect(prompt).toContain('automation instruction is not an explicit end-user request')
        expect(prompt).toContain('MUST set taskOrigin to "assistant_suggestion", never "user_request"')
        expect(prompt).toContain('concrete email-based reason')
        expect(prompt).toContain('do not merely restate the task title or give a generic reason')
        expect(prompt).toContain('"Email from [sender name] ([sender email]): [one-line summary]"')
