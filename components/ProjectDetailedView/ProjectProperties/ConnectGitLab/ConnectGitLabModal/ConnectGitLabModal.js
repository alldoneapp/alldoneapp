...
import { CONNECT_GITLAB_MODAL_ID, removeModal, storeModal } from '../../../../ModalsManager/modalsManager'
...
export default function ConnectGitLabModal({ project, closePopover, onConnectionChange }) {
    ...
    const [baseBranch, setBaseBranch] = useState(project.gitlabBaseBranch || '')
    ...
    useEffect(() => {
        storeModal(CONNECT_GITLAB_MODAL_ID)
        return () => removeModal(CONNECT_GITLAB_MODAL_ID)
    }, [])

    useEffect(() => {
        let mounted = true
        getGitlabUserConnection(projectId, userId).then(c => {
    ...
    const onConnect = async () => {
        setError('')
        setSuccess('')
        if (!repoUrl.trim()) return setError(translate('Please enter the GitLab repository URL.'))
        if (!token.trim()) return setError(translate('Please paste a GitLab access token.'))
        setProcessing(true)
        try {
            const result = await connectGitlabRepo({
                projectId,
                token: token.trim(),
            ...
            setSuccess(result.warning || translate('GitLab repository connected.'))
            if (onConnectionChange) onConnectionChange({ connected: true, repoUrl: repoUrl.trim() })
        } catch (e) {
            setError((e && e.message) || translate('Could not connect the GitLab repository.'))
        } finally {
            setProcessing(false)
        }
    }
    ...
    const onDisconnect = async () => {
        ...
        setProcessing(true)
        try {
            await disconnectGitlabRepo({ projectId })
            setConnection(null)
            setToken('')
            setSuccess(translate('Your GitLab token was removed from this project.'))
            if (onConnectionChange) onConnectionChange({ connected: false })
        } catch (e) {
            setError((e && e.message) || translate('Could not disconnect GitLab.'))
        } finally {
            setProcessing(false)
        }
    }
        ...
        <View style={[localStyles.container, { width: containerWidth, maxHeight: containerMaxHeight }]}>
            ...
            >
                <ModalHeader
                    ...
                    title={translate('GitLab repository')}
                    description={translate('GitLab repository description')}
                />
                    ...
                    <View style={localStyles.statusCard}>
                        ...
                        {connection.canPush === false && (
                            <Text style={localStyles.warningText}>
                                {translate('This token is read-only and cannot open Merge Requests.')}
                            </Text>
                        )}
                    </View>
                ) : null}
                ...
                <TextInput
                    ...
                    placeholder={isConnected ? translate('Paste a new token to replace') : 'glpat-…'}
                    placeholderTextColor={colors.Text03}
                    autoCapitalize={'none'}
                    autoCorrect={false}
                ...
                <Text style={localStyles.help}>
                    {translate(
                        'Use a Project Access Token with api + write_repository scope (Developer role or higher).'
                    )}
                </Text>
