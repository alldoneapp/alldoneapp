...
describe('the completed sweep on the open-tasks board (AT-2492)', () => {
    ...
    describe('in All Projects', () => {
        ...
        it('animates the project away after bypassing its only suggested task with a stale count', async () => {
            const tree = await render(buildState({ todayIsEmpty: false, todayCount: 1 }))

            await act(async () => {
                publishProjectTaskCompletion({ projectId: PROJECT, taskId: 'suggested-task' })
            })
            // The open-task listener has removed the only suggested row and therefore the project,
            // while the independent sidebar listener still reports one.
            await update(tree, buildState({ todayIsEmpty: true, todayCount: 1 }))
