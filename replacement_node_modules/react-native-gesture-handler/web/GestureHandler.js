...
// Alldone patch: react-native-web 0.19+ removed findNodeHandle (it now throws
// unconditionally), which made every gesture-handler attachment die in
// componentDidMount. With modern RNW, refs to host components ARE the DOM
// nodes, so resolve the node directly and fall back through the legacy
// accessor chain for wrapped/class refs.
const resolveDomNode = ref => {
    if (ref == null) return null
    if (typeof HTMLElement !== 'undefined' && ref instanceof HTMLElement) return ref
    if (typeof ref.getDOMNode === 'function') return ref.getDOMNode()
    if (typeof ref.getNode === 'function') return resolveDomNode(ref.getNode())
    if (ref.current != null) return resolveDomNode(ref.current)
    return null
}
...
class GestureHandler {
    ...
    sendEvent = nativeEvent => {
        const { onGestureHandlerStateChange: onHandlerStateChange, onGestureHandlerEvent: onGestureEvent } =
            (this.ref && this.ref.props) ||
            (this.propsRef && {
                onGestureHandlerStateChange: this.propsRef._onGestureHandlerStateChange,
                onGestureHandlerEvent: this.propsRef._onGestureHandlerEvent,
            }) ||
            {}
    ...
    setView(ref, propsRef) {
        ...
        // Alldone patch: with react-native-web 0.19+ the ref is the DOM node, so
        // event callbacks can no longer be read off ref.props. The handler wrapper
        // component is threaded through attachGestureHandler as propsRef and its
        // dispatcher methods back the same cloned props the legacy path read.
        this.propsRef = propsRef
        this.ref = ref

        this.view = resolveDomNode(ref)
        if (this.view == null) {
            // Without a DOM node there is nothing to attach Hammer to; behave like
            // the ref-cleared path instead of crashing the mount.
            this.destroy()
            return
        }
