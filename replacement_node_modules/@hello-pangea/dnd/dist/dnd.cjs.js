...
var moveToNextIndex = ({
    ...
    !wasAt
        ? process.env.NODE_ENV !== 'production'
            ? invariant(false, 'Cannot move in direction without previous impact location')
            : invariant(false)
        : void 0
...
const goAfter = ({ axis, moveRelativeTo, isMoving }) =>
    patch(
        axis.line,
...
const goBefore = ({ axis, moveRelativeTo, isMoving }) =>
    patch(
        axis.line,
        moveRelativeTo.marginBox[axis.start] - distanceFromEndToBorderBoxCenter(axis, isMoving),
        getCrossAxisBorderBoxCenter(axis, moveRelativeTo.marginBox, isMoving)
    )
const goIntoStart = ({ axis, moveInto, isMoving }) =>
    patch(
        axis.line,
...
const addPlaceholder = (droppable, draggable, draggables) => {
    ...
    !!droppable.subject.withPlaceholder
        ? process.env.NODE_ENV !== 'production'
            ? invariant(false, 'Cannot add placeholder size to a subject when it already has one')
            : invariant(false)
        : void 0
...
const removePlaceholder = droppable => {
    ...
    !added
        ? process.env.NODE_ENV !== 'production'
            ? invariant(false, 'Cannot remove placeholder form subject when there was none')
            : invariant(false)
        : void 0
    const frame = droppable.frame
    if (!frame) {
    ...
    !oldMaxScroll
        ? process.env.NODE_ENV !== 'production'
            ? invariant(false, 'Expected droppable with frame to have old max frame scroll when removing placeholder')
            : invariant(false)
        : void 0
...
var getCombineImpact = ({
    ...
    const impact = {
        ...
        at: {
            type: 'COMBINE',
            combine: {
                // Alldone patch (carried over from the old react-beautiful-dnd patch):
                // expose the combine target's index so DragHelper.onDragEnd can sort a
                // task dropped ONTO another task. Upstream omits it from the payload.
                index: combineWith.descriptor.index,
...
var offsetDraggable = ({ draggable, offset, initialWindowScroll }) => {
...
var reducer = (state = idle$2, action) => {
    ...
    if (action.type === 'PUBLISH_WHILE_DRAGGING') {
        !(state.phase === 'COLLECTING' || state.phase === 'DROP_PENDING')
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, `Unexpected ${action.type} received in phase ${state.phase}`)
                : invariant(false)
            : void 0
    ...
    if (action.type === 'MOVE') {
        ...
        !isMovementAllowed(state)
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, `${action.type} not permitted in phase ${state.phase}`)
                : invariant(false)
            : void 0
    ...
    if (action.type === 'UPDATE_DROPPABLE_IS_ENABLED') {
        ...
        !isMovementAllowed(state)
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, `Attempting to move in an unsupported phase ${state.phase}`)
                : invariant(false)
            : void 0
        ...
        !target
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, `Cannot find Droppable[id: ${id}] to toggle its enabled state`)
                : invariant(false)
            : void 0
    ...
    if (action.type === 'UPDATE_DROPPABLE_IS_COMBINE_ENABLED') {
        ...
        !target
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, `Cannot find Droppable[id: ${id}] to toggle its isCombineEnabled state`)
                : invariant(false)
            : void 0
    ...
    if (action.type === 'MOVE_BY_WINDOW_SCROLL') {
        ...
        !state.isWindowScrollAllowed
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, 'Window scrolling is currently not supported for fixed lists')
                : invariant(false)
            : void 0
    ...
    if (
        ...
        !(state.phase === 'DRAGGING')
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, `${action.type} received while not in DRAGGING phase`)
                : invariant(false)
            : void 0
    ...
    if (action.type === 'DROP_PENDING') {
        ...
        !(state.phase === 'COLLECTING')
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, 'Can only move into the DROP_PENDING phase from the COLLECTING phase')
                : invariant(false)
            : void 0
        const newState = {
            ...state,
...
const dropMiddleware = ({ getState, dispatch }) => next => action => {
    ...
    !!isWaitingForDrop
        ? process.env.NODE_ENV !== 'production'
            ? invariant(false, 'A DROP action occurred while DROP_PENDING and still waiting')
            : invariant(false)
        : void 0
    !(state.phase === 'DRAGGING' || state.phase === 'DROP_PENDING')
        ? process.env.NODE_ENV !== 'production'
...
var getPublisher = (getResponders, announce) => {
    ...
    const beforeCapture = (draggableId, mode) => {
        !!dragging
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, 'Cannot fire onBeforeCapture as a drag start has already been published')
                : invariant(false)
            : void 0
    ...
    const beforeStart = (critical, mode) => {
        !!dragging
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, 'Cannot fire onBeforeDragStart as a drag start has already been published')
                : invariant(false)
            : void 0
    ...
    const update = (critical, impact) => {
        ...
        !dragging
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, 'Cannot fire onDragMove when onDragStart has not been called')
                : invariant(false)
            : void 0
    ...
    const drop = result => {
        !dragging
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, 'Cannot fire onDragEnd when there is no matching onDragStart')
                : invariant(false)
            : void 0
        dragging = null
...
const dropAnimationFinishMiddleware = store => next => action => {
    ...
    !(state.phase === 'DROP_ANIMATING')
        ? process.env.NODE_ENV !== 'production'
            ? invariant(false, 'Cannot finish a drop animating when no drop is occurring')
            : invariant(false)
        : void 0
    store.dispatch(
        completeDrop({
...
var autoScroll = autoScroller => store => next => action => {
    ...
    if (action.type === 'INITIAL_PUBLISH') {
        ...
        !(state.phase === 'DRAGGING')
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, 'Expected phase to be DRAGGING after INITIAL_PUBLISH')
                : invariant(false)
            : void 0
...
const composeEnhancers =
    ...
    window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__
        ...
        : redux.compose
var createStore = ({ dimensionMarshal, focusMarshal, styleMarshal, getResponders, announce, autoScroller }) =>
    redux.createStore(
        reducer,
        composeEnhancers(
...
var createDimensionMarshal = (registry, callbacks) => {
    ...
    const updateDroppableIsEnabled = (id, isEnabled) => {
        !registry.droppable.exists(id)
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, `Cannot update is enabled flag of Droppable ${id} as it is not registered`)
                : invariant(false)
            : void 0
        if (!collection) {
            return
    ...
    const updateDroppableIsCombineEnabled = (id, isCombineEnabled) => {
        ...
        !registry.droppable.exists(id)
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, `Cannot update isCombineEnabled flag of Droppable ${id} as it is not registered`)
                : invariant(false)
            : void 0
    ...
    const updateDroppableScroll = (id, newScroll) => {
        ...
        !registry.droppable.exists(id)
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, `Cannot update the scroll on Droppable ${id} as it is not registered`)
                : invariant(false)
            : void 0
    ...
    const subscriber = event => {
        !collection
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, 'Should only be subscribed when a collection is occurring')
                : invariant(false)
            : void 0
    ...
    const startPublishing = request => {
        !!collection
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, 'Cannot start capturing critical dimensions as there is already a collection')
                : invariant(false)
            : void 0
...
var createJumpScroller = ({ move, scrollDroppable, scrollWindow }) => {
    ...
    const jumpScroller = state => {
        ...
        !destination
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, 'Cannot perform a jump scroll when there is no destination')
                : invariant(false)
            : void 0
...
function useStyleMarshal(contextId, nonce) {
    ...
    const dropping = useMemoOne.useCallback(
        reason => {
            if (reason === 'DROP') {
...
function useFocusMarshal(contextId) {
    ...
    const tryGiveFocus = useMemoOne.useCallback(
    ...
    const tryRestoreFocusRecorded = useMemoOne.useCallback(
...
function useMouseSensor(api) {
    ...
    const listenForCapture = useMemoOne.useCallback(
    ...
    const bindCapturingEvents = useMemoOne.useCallback(
    ...
    const startPendingDrag = useMemoOne.useCallback(
        ...
        () => ({
            ...
            fn: function onKeyDown(event) {
                ...
                function stop() {
                    !isCapturing
                        ? process.env.NODE_ENV !== 'production'
                            ? invariant(false, 'Cannot stop capturing a keyboard drag when not capturing')
                            : invariant(false)
                        : void 0
                    isCapturing = false
                    unbindEventsRef.current()
...
function useTouchSensor(api) {
    ...
    const startDragging = useMemoOne.useCallback(
    ...
    useDev(() => {
        ...
        useDevSetupWarning(() => {
            !(previousRef.current.length === sensorHooks.length)
                ? process.env.NODE_ENV !== 'production'
                    ? invariant(false, 'Cannot change the amount of sensor hooks after mounting')
                    : invariant(false)
                : void 0
        })
    })
...
function useSensorMarshal({ contextId, store, registry, customSensors, enableDefaultSensors }) {
    ...
    const tryAbandonLock = useMemoOne.useCallback(
    ...
    const canGetLock = useMemoOne.useCallback(
        draggableId => {
            return canStart({
                lockAPI,
                registry,
    ...
    const tryGetLock = useMemoOne.useCallback(
    ...
    const findClosestDraggableId = useMemoOne.useCallback(
        event => tryGetClosestDraggableIdFromEvent(contextId, event),
        [contextId]
    )
    const findOptionsForDraggable = useMemoOne.useCallback(
        id => {
    ...
    const tryReleaseLock = useMemoOne.useCallback(
...
function App(props) {
    ...
    const marshalCallbacks = useMemoOne.useMemo(
        () =>
    ...
    const autoScroller = useMemoOne.useMemo(
        () =>
            createAutoScroller({
                scrollWindow,
    ...
    const store = useMemoOne.useMemo(
        () =>
            createStore({
                announce,
                autoScroller,
...
function useDraggablePublisher(args) {
    ...
    const getDimension = useMemoOne.useCallback(
        windowScroll => {
    ...
    useDev(() => {
        ...
        useDevSetupWarning(() => {
            !(isClone === initialRef.current)
                ? process.env.NODE_ENV !== 'production'
                    ? invariant(false, 'Draggable isClone prop value changed during component life')
                    : invariant(false)
                : void 0
        }, [isClone])
    })
...
const Draggable = props => {
    ...
    const dragHandleProps = useMemoOne.useMemo(
        () =>
            isEnabled
                ? {
                      tabIndex: 0,
    ...
    const onMoveEnd = useMemoOne.useCallback(
        event => {
...
function getDraggableSelector() {
    ...
    const getMemoizedSnapshot = memoizeOne(
        (mode, isClone, draggingOver = null, combineWith = null, dropping = null) => ({
            isDragging: true,
            isClone,
    ...
    const getMemoizedProps = memoizeOne(
        (offset, mode, dimension, isClone, draggingOver = null, combineWith = null, forceShouldAnimate = null) => ({
            mapped: {
                type: 'DRAGGING',
                dropping: null,
                draggingOver,
...
function useDroppablePublisher(args) {
    ...
    const memoizedUpdateScroll = useMemoOne.useMemo(
        () =>
            memoizeOne((x, y) => {
    ...
    const getDimensionAndWatchScroll = useMemoOne.useCallback(
        (windowScroll, options) => {
            !!whileDraggingRef.current
                ? process.env.NODE_ENV !== 'production'
                    ? invariant(false, 'Cannot collect a droppable while a drag is occurring')
                    : invariant(false)
                : void 0
    ...
    const getScrollWhileDragging = useMemoOne.useCallback(() => {
        ...
        !(dragging && closest)
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, 'Can only recollect Droppable client for Droppables that have a scroll container')
                : invariant(false)
            : void 0
    ...
    const scroll = useMemoOne.useCallback(change => {
        ...
        !closest
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, 'Cannot scroll a droppable with no closest scrollable')
                : invariant(false)
            : void 0
...
const Placeholder = props => {
    ...
    const onSizeChangeEnd = useMemoOne.useCallback(
        event => {
...
const virtual = [
    function hasClone({ props }) {
        !props.renderClone
            ? process.env.NODE_ENV !== 'production'
                ? invariant(false, 'Must provide a clone render function (renderClone) for virtual lists')
                : invariant(false)
            : void 0
    },
...
const Droppable = props => {
    ...
    const placeholder = useMemoOne.useMemo(
        () =>
            React.createElement(
                AnimateInOut,
                {
