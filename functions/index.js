...
exports.setDefaultVmApprovalPolicy = onCall(
    {
        timeoutSeconds: 30,
        memory: '256MiB',
        region: 'europe-west1',
    ...
    async request => {
        const { data, auth } = request
        if (!auth) throw new HttpsError('permission-denied', 'Authentication required')
        const { setDefaultVmApprovalPolicy } = require('./Assistant/vmAgentSettings')
        return await setDefaultVmApprovalPolicy({ userId: auth.uid, policy: data && data.policy })
    }
)
...
// LEGACY RUN VM JOB - retained only as a manual rollback target. New tasks always
// launch the detached Cloud Run Job directly.
...
exports.reconcileVmCloudRunLaunches = onSchedule(
    ...
    async () => {
        ...
        const { reconcileOrphanedQueuedVmJobs } = require('./Assistant/vmThreadQueue')
        const [launches, workers, orphanedQueues] = await Promise.all([
        ...
        if (
            ...
            workers.reconciled ||
            workers.errors ||
            orphanedQueues.reconciled ||
            orphanedQueues.errors
        ) {
            console.log('🖥️ VM JOB: Cloud Run reconciliation complete', {
                launches,
                workers,
                orphanedQueues,
            })
...
exports.endAssistantBrowserCallSecondGen = onCall(
    { region: 'europe-west1', timeoutSeconds: 30, memory: '512MiB' },
