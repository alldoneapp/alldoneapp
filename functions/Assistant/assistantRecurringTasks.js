...
const { createTaskCreatedFeed } = require('../Feeds/tasksFeeds')
    ...
    try {
        ...
        try {
            ...
            if (!existingTaskDoc.exists) {
                ...
                // Create a regular task using the assistant task data
                const regularTask = {
                    name: task.name,
                    extendedName: task.extendedName || task.name,
                    userId: assistantId,
                    userIds: [assistantId],
                ...
                await admin.firestore().doc(`items/${projectId}/tasks/${uniqueId}`).set(regularTask)
                console.log('Successfully created regular task with same ID as ChatObject:', { uniqueId, projectId })

                // --- Create Feed Entry ---
                try {
                    // Pass the fetched feedUserData (creator, assistant, or system)
                    // Pass null for batch (function handles it), false for notification
                    await createTaskCreatedFeed(projectId, regularTask, uniqueId, null, feedUserData, false)
                    console.log('Successfully created task feed entry:', { uniqueId, projectId })
                } catch (feedError) {
                    console.error('Error creating task feed entry:', {
                        error: feedError,
                        uniqueId,
                        projectId,
                        assistantTaskId: taskId, // original assistant task id
                        errorMessage: feedError.message,
                        errorCode: feedError.code,
                    })
                    // Continue execution even if feed creation fails
                }
                // --- End Feed Entry ---
            } else {
