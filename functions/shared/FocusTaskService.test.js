...
/**
 * AT-2496 — the imminent/running calendar exception must actually fire.
 *
 * Phase 1 used to pre-filter the query with `sortIndex >= now && sortIndex < now + 15min`, which
 * was only ever correct while `sortIndex` HELD THE EVENT START — the encoding AT-2259 deliberately
 * retired. Today `generateCalendarTaskSortIndex()` stamps the sync-time "now" once at creation, so
 * the value is hours in the PAST by the time the meeting starts and the query matched nothing at
 * all. Production confirmed it: 0 of 119 real calendar tasks on the reporting account had a
 * `sortIndex` that could ever fall in that window.
 *
 * Every fixture here therefore carries the PRODUCTION shape — `sortIndex` = sync time, `dueDate` =
 * the event day — which is exactly what the pre-existing calendar test did not do (it set
 * `sortIndex: soon.valueOf()`, the retired encoding, and so exercised a path production could not
 * reach).
 */
describe('FocusTaskService calendar focus window (AT-2496)', () => {
    const userId = 'user-1'
    const currentProjectId = 'project-1'
    const otherProjectId = 'project-2'
    const now = moment()
    ...
    // What the calendar sync actually writes: the task was created hours before the meeting.
    const syncedAt = now.clone().subtract(6, 'hours').valueOf()
    const eventDay = now.clone().startOf('day').valueOf()

    const calendarTask = (id, startOffsetMinutes, durationMinutes = 30, overrides = {}) => {
        const startMs = now.valueOf() + startOffsetMinutes * MINUTE
        return {
            id,
            name: id,
            userId,
            ...
            calendarData: {
                eventId: `event-${id}`,
                start: { dateTime: moment(startMs).toISOString() },
                end: { dateTime: moment(startMs + durationMinutes * MINUTE).toISOString() },
            },
            ...overrides,
        }
    }

    const ordinaryTask = (id, sortIndex) => ({
        id,
        name: id,
        userId,
        done: false,
        ...
        userIds: [userId],
        currentReviewerId: userId,
        dueDate: now.clone().subtract(1, 'hour').valueOf(),
        sortIndex,
    })
    ...
    const createService = ({ docs = {}, collections = {} }) => {
        const service = new FocusTaskService({
            database: createMockDatabase({
                ...
                collections: { [`items/${otherProjectId}/tasks`]: [], ...collections },
            }),
            moment,
        })
        service.setNewFocusTask = jest.fn().mockResolvedValue()
    ...
    test('selects a meeting starting in five minutes over the top ordinary task', async () => {
        const service = createService({
            collections: {
                [`items/${currentProjectId}/tasks`]: [
                    ordinaryTask('ordinary-top', 9_000_000),
                    calendarTask('meeting-soon', 5),
                ],
            },
        })
        ...
        expect(result.id).toBe('meeting-soon')
        expect(service.setNewFocusTask).toHaveBeenCalledWith(
            userId,
            currentProjectId,
            expect.objectContaining({ id: 'meeting-soon' }),
            {}
        )
    })

    test('finds the meeting in another project, not just the search project', async () => {
        const service = createService({
            collections: {
                [`items/${currentProjectId}/tasks`]: [ordinaryTask('ordinary-top', 9_000_000)],
                [`items/${otherProjectId}/tasks`]: [calendarTask('meeting-elsewhere', 3)],
            },
        })

        const result = await service.findAndSetNewFocusTask(userId, currentProjectId, null, null, null, null)

        expect(result.id).toBe('meeting-elsewhere')
        expect(result.projectId).toBe(otherProjectId)
    })

    test('selects a meeting that is already running', async () => {
        const service = createService({
            collections: {
                [`items/${currentProjectId}/tasks`]: [
                    ordinaryTask('ordinary-top', 9_000_000),
                    calendarTask('meeting-running', -10, 60),
                ],
            },
        })
        ...
        expect(result.id).toBe('meeting-running')
    })

    test('prefers an upcoming meeting over one already running', async () => {
        const service = createService({
            collections: {
                [`items/${currentProjectId}/tasks`]: [
                    calendarTask('meeting-running', -10, 60),
                    calendarTask('meeting-next', 8),
                ],
            },
        })
        ...
        expect(result.id).toBe('meeting-next')
    })

    test('leaves a long block alone once its running window has expired', async () => {
        // A 5-hour "Work Blocker" started 90 minutes ago. Uncapped it would own the focus task all
        // afternoon, so every task finished inside the block handed focus straight back to it.
        const service = createService({
            collections: {
                [`items/${currentProjectId}/tasks`]: [
                    ordinaryTask('ordinary-top', 9_000_000),
                    calendarTask('work-blocker', -90, 300),
                ],
            },
        })
        ...
        expect(result.id).toBe('ordinary-top')
    })

    test('never selects an all-day event that merely covers today', async () => {
        const allDay = calendarTask('birthday', 0, 0, {
            calendarData: {
                eventId: 'event-birthday',
                start: { date: now.clone().startOf('day').format('YYYY-MM-DD') },
                end: { date: now.clone().add(1, 'day').startOf('day').format('YYYY-MM-DD') },
            },
        })

        const service = createService({
            collections: {
                [`items/${currentProjectId}/tasks`]: [ordinaryTask('ordinary-top', 9_000_000), allDay],
            },
        })
    ...
    test('still honours excludeTaskId for the meeting that just left focus', async () => {
        const service = createService({
            collections: {
                [`items/${currentProjectId}/tasks`]: [
                    ordinaryTask('ordinary-top', 9_000_000),
        ...
        const result = await service.findAndSetNewFocusTask(userId, currentProjectId, null, 'meeting-soon', null, null)

        expect(result.id).toBe('ordinary-top')
    })

    test('does not query on sortIndex any more — it is a list position, not an event time', async () => {
        const database = createMockDatabase({
            docs: { [`users/${userId}`]: { id: userId, defaultProjectId: currentProjectId } },
            collections: {
                [`items/${currentProjectId}/tasks`]: [calendarTask('meeting-soon', 5)],
                [`items/${otherProjectId}/tasks`]: [],
            },
        })

        const service = new FocusTaskService({ database, moment })
        service.setNewFocusTask = jest.fn().mockResolvedValue()

        await service.findAndSetNewFocusTask(userId, currentProjectId, null, null, null, null)
