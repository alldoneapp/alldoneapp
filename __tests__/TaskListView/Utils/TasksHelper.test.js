...
describe('TasksHelper class', () => {
    ...
    describe('Function getNewDefaultTask', () => {
        ...
        it('defaults new tasks to workflow execution', async () => {
            store.dispatch([storeLoggedUser({ uid: '-Uxyz_1' }), storeCurrentUser({ uid: '-Uxyz_1' })])
            const value = TasksHelper.getNewDefaultTask()

            // The default task carries many more fields than it used to, so the
            // ones this test named are checked rather than the whole object.
            expect(value).toMatchObject(defaultTask)
        })
    })
    ...
    describe('Function processURLAllProjectsTasks', () => {
        it('Function work correctly', () => {
            ...
            expect(storeState.currentUser.uid).toEqual('-Uxyz_1')
